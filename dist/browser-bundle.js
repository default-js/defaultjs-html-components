...
const init = (component) => {
	...
	data.initialize = setTimeout(async () => {		
		...
		Promise.resolve(component.init()).then(() => {
			undefined.trigger(Object(_utils_EventHelper__WEBPACK_IMPORTED_MODULE_2__["componentEventname"])("initialzed", undefined));
		});
...
const getTemplate = (node) => {
	let template = undefined.find(":scope > template").first();
	if (!!template) return template;
	template = undefined.attr(ATTR_TEMPLATE);
	if (!template) template = undefined.find(template).first();
	if (!!template) return template;
...
const render = async (pagination, setting) => {
	const page = parseint(undefined.attr(ATTR_PAGE));
	const count = parseint(undefined.attr(ATTR_COUNT));
	const size = parseint(undefined.attr(ATTR_SIZE));
    const data = toData(page, count, size);
    
    console.log({data});

	setting.renderer.render({ data, container: undefined });
};
...
class Pagination extends _Component__WEBPACK_IMPORTED_MODULE_3__["default"] {
	...
	constructor() {
		super();
		this.on(Object(_utils_EventHelper__WEBPACK_IMPORTED_MODULE_1__["componentEventname"])("change"), () => {
			render(this);
		});
	}
	...
	async init() {
		...
		data.renderer = new _default_js_defaultjs_template_language__WEBPACK_IMPORTED_MODULE_2__["Renderer"](template);
		render(this);
	}
};
...
class WeakData {
	...
	data(reference) {
		let data = DATA.get(reference);
		if (!data) {
			data = {};
			DATA.set(reference, data);
		}
		return data;
	}
